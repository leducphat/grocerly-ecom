...
| # | Vấn đề | Ảnh hưởng |
|---|---|---|
| 1 | ~~Thiếu tầng unit test~~ 🔸 **Đã đóng 2026-08-26** — **513 test**, ba tầng: unit thuần (`SimpleTestCase`, không dựng database) / mức model / hồi quy ở mức HTTP. Nhóm 2.6 xong toàn bộ | Không còn mục nào phải kiểm thử tay. Chỗ mỏng nhất còn lại là `store_api` (11 test) và các view thống kê ở `useradmin` |
...
| 10 | Django admin ghi thẳng `paid_status` và M2M `coupons` của đơn hàng | `CartOrderAdmin` không khai `fields`/`readonly_fields` nên form có đủ hai ô đó. Tick `paid_status` là **không** đi qua `CartOrder.confirm_paid()`, nên `used_count` của mã giảm giá không tăng và `usage_limit` mất hiệu lực im lặng. Tệ hơn: sau đó nhân viên chuyển đơn sang `delivered` thì `confirm_paid()` thấy `paid_status` đã bật nên bỏ qua — lượt đó mất vĩnh viễn. Chưa vá vì khóa `paid_status` lại thì quản trị viên hết đường sửa một đơn ghi nhận sai; hướng đúng là override `save_model` để định tuyến qua `confirm_paid()` |
| 9 | `Coupon.code` **không unique** | Hai bản ghi cùng mã thì `checkout()` chỉ áp được bản mới nhất (`.order_by('-id').first()`), còn chốt chống áp trùng so theo **object** chứ không theo chuỗi `code` — nên hai bản ghi cùng mã vẫn áp chồng lên một đơn được. Không thêm ràng buộc unique vì trên model xóa mềm thì `unique` phẳng chặn luôn việc tạo lại mã đã xóa, còn `UniqueConstraint` có điều kiện thì **migration có thể fail trên dữ liệu Neon** mà không kiểm trước được |
