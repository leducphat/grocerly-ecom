...
class LoginFailureTests(TestCase):
    ...
    def test_an_unknown_email_does_not_start_a_session(self):
        """Đường này đi qua `except:` trần ở `userauths/views.py` — [S-07](../../docs/SECURITY.md).

        Test khẳng định **kết quả** (không có phiên, có thông báo) chứ không khẳng định
        cách bắt lỗi, nên nó vẫn xanh sau khi bước 2.5 thay `except:` bằng
        `except User.DoesNotExist:`. Đó là chủ ý: lưới an toàn cho bản vá đó, chứ không
        phải cái chốt lại hành vi sai.
        """
        response = self.client.post(
